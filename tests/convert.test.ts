...
function sip002(method: string, password: string, host: string, port: number, name: string): string {
...
describe("convertSubscriptionText", () => {
  ...
  it("sanitizes unsafe policy-name separators and deduplicates the sanitized names", () => {
    ...
    expect(convertSubscriptionText(input).nodes.map((node) => node.name)).toEqual(["Tokyo Node", "Tokyo Node (2)"]);
  });

  it("uses a safe fallback when a VLESS fragment is missing or malformed", () => {
    const result = convertSubscriptionText("vless://private-id@example.com:443?type=tcp&security=reality&pbk=private-key");
